---
title: "Scan Global Synchronizer Connectivity API"
description: "Discover Scan and sequencer endpoints across all SVs"
---
...
{/* NETWORKVARS_START source="/snippets/networkvars/sdks-tools/api-reference/splice-scan-gs-connectivity-api-1.mdx" */}
<Tabs>

<Tab title="DevNet (0.8.0)">

Every Scan can list all approved SV scans connected to the network. For example, query from /v0/scans from <a href="https://scan.sv-1.dev.global.canton.network.sync.global">https://scan.sv-1.dev.global.canton.network.sync.global</a>, and the response will be something like
...
Every Scan can list all approved SV scans connected to the network. For example, query from /v0/scans from <a href="https://scan.sv-1.test.global.canton.network.sync.global">https://scan.sv-1.test.global.canton.network.sync.global</a>, and the response will be something like
...
Every Scan can list all approved SV scans connected to the network. For example, query from /v0/scans from <a href="https://scan.sv-1.global.canton.network.sync.global">https://scan.sv-1.global.canton.network.sync.global</a>, and the response will be something like
...
In any Canton deployment, [each party is hosted on a participant](/global-synchronizer/production-operations/party-management#party-management). This can be accessed through Scan with /v0/domains/{domain_id}/parties/{party_id}/participant-id.
